...
import styles from './RankingsTableSection.module.css';
...
export function RankingsTableSection({ projectId }: Props) {
  ...
  const triggerSingleCheck = useTriggerSingleKeywordCheck();

  const keywords = data?.keywords || [];
  const totalPages = data?.totalPages || 1;
  const gscConnected = data?.gscConnected || false;
  // Only treat GSC as "active" for this project if at least one keyword
  // actually has GSC data — otherwise the badge/columns are misleading.
  const gscHasData = keywords.some((k) => k.gsc !== null);
  ...
  const handleRecheckOne = async (keywordId: string, keyword: string) => {
    setRecheckingIds((s) => new Set(s).add(keywordId));
    try {
      const res: any = await triggerSingleCheck.mutateAsync({
        url: `/projects/${projectId}/position-tracking/keywords/${keywordId}/check-now`,
        body: {},
      });
        ...
        <h2 className={styles.sectionTitle}>
          ...
          {gscConnected && gscHasData && (
            <span
              ...
              title="Real impressions/clicks/CTR from Google Search Console (last 28 days)"
            >
              ✓ Real GSC data
            </span>
          )}
          {gscConnected && !gscHasData && (
            <span
              ...
              title="Google Search Console is connected, but this domain has no search data yet (new site or not verified for this account)."
            >
              GSC connected — no data for this domain yet
            </span>
          )}
        </h2>
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  ...
                  <th title="Country / device the keyword is tracked in">Market</th>
                  ...
                  {gscHasData && <th title="Real clicks from Search Console (last 28d)">GSC Clicks</th>}
                  {gscHasData && <th title="Real impressions from Search Console (last 28d)">GSC Impr.</th>}
                  {gscHasData && <th title="Real click-through rate from Search Console (last 28d)">GSC CTR</th>}
                  <th>URL</th>
                  <th>Tags</th>
                  <th>Actions</th>
                </tr>
                ...
                {keywords.map((kw: TrackedKeywordWithPosition) => {
                  ...
                  return (
                    <tr key={kw.id}>
                      ...
                      {gscHasData && (
                        <td title="Real clicks from GSC">
                          {kw.gsc ? kw.gsc.clicks.toLocaleString() : <span className={styles.muted}>--</span>}
                        </td>
                      )}
                      {gscHasData && (
                        <td title="Real impressions from GSC">
                          {kw.gsc ? formatVolume(kw.gsc.impressions) : <span className={styles.muted}>--</span>}
                        </td>
                      )}
                      {gscHasData && (
                        <td title="Real CTR from GSC">
                          {kw.gsc ? `${(kw.gsc.ctr * 100).toFixed(1)}%` : <span className={styles.muted}>--</span>}
                        </td>
                      )}
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                          <button
                            ...
                            onClick={() => handleRecheckOne(kw.id, kw.keyword)}
                            disabled={recheckingIds.has(kw.id)}
                            title="Re-check this keyword now (uses 1 SerpAPI credit)"
                          >
                            {recheckingIds.has(kw.id) ? '…' : '⟳'}
                          </button>
                          <button
