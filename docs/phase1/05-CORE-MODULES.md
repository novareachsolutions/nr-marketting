...
```
1. User clicks "Connect GitHub" on Settings page (or during project creation)
2. Frontend redirects to backend with JWT token in query param:
   GET /api/github/authorize?projectId={id}&token={jwt}
3. Backend verifies JWT, redirects to GitHub OAuth consent:
...
5. GitHub redirects to callback → backend exchanges code for access token
6. Backend redirects to frontend settings page with ghToken in URL:
   /dashboard/projects/{id}/settings?ghToken={access_token}
7. Frontend reads ghToken, fetches user's repos via:
   GET /api/projects/{id}/github/repos?accessToken={token}
8. User selects a repository from the list
9. Backend on connect:
   a. Store AES-256-GCM encrypted access token
   b. Detect default branch
   c. Auto-detect deploy platform:
      - Check for vercel.json → VERCEL
      - Check for netlify.toml → NETLIFY
      - Check for CNAME file → GITHUB_PAGES (custom domain)
      - Check GitHub Pages API → GITHUB_PAGES
   d. Create webhook for push + deployment_status events
...
| Fix issues | Manual | **Create PRs with fixes** (meta tags, alt text, canonical, structured data, og:image, etc.) |
...
#### Auto-Fix via Pull Request (IMPLEMENTED)

```
User sees crawl issue (e.g. "Missing canonical URL tag" or "Missing structured data")
User clicks "Auto-Fix" button on the audit details page

1. Backend fetches full repo file tree:
   GET /repos/{owner}/{repo}/git/trees/{branch}?recursive=1

2. Smart file matching finds the source file:
   - Matches URL path to file (app/page.tsx, pages/index.tsx, etc.)
   - Supports Next.js (app router + pages router), Gatsby, Astro, static HTML, Markdown
   - Falls back to fuzzy slug matching across all source files

3. Backend reads the source file:
   GET /repos/{owner}/{repo}/contents/{detected_path}

4. AI (GPT-4o-mini) generates the fix:
   - Receives the full file content + issue details
   - Returns the complete fixed file preserving existing code
   - Knows how to add canonical tags, JSON-LD, og:image, meta tags, etc.

5. Backend creates branch + commits fix + opens PR:
   POST /repos/{owner}/{repo}/git/refs         (create branch)
   PUT  /repos/{owner}/{repo}/contents/{path}   (commit fix)
   POST /repos/{owner}/{repo}/pulls             (open PR)

6. Frontend shows "View PR" link to user
7. When user merges PR → webhook fires → auto re-crawl → issue resolved
```

**Fixable Issue Types (14):**
...
- MISSING_H1, IMAGE_MISSING_ALT, LOW_WORD_COUNT
- MISSING_CANONICAL, MISSING_STRUCTURED_DATA, MISSING_OG_IMAGE
- NO_DIRECT_ANSWERS, WEAK_EEAT_SIGNALS

#### Deploy Detection via Webhook (IMPLEMENTED)

```
On connect, backend creates a webhook on the repo:
  POST /repos/{owner}/{repo}/hooks
  {
    events: ["push", "deployment_status"],
    config: {
      url: "{API_BASE_URL}/github/webhook",
      content_type: "json",
      secret: "{random_32_byte_hex}"
    }
  }

Webhook secret stored encrypted (AES-256-GCM) in DB.

When push to default branch is detected:
  1. Verify x-hub-signature-256 using HMAC-SHA256 with stored secret
  2. Wait 2 minutes (for deploy to propagate)
  3. Auto-trigger site re-crawl for the project
  4. Log "Deploy detected" + "Auto re-crawl triggered"

On disconnect, webhook is deleted from GitHub:
  DELETE /repos/{owner}/{repo}/hooks/{webhook_id}

Note: Webhook requires a publicly reachable API_BASE_URL.
In production this works automatically. For local dev, use the
"Simulate Deploy" button on the Settings page instead.
```

#### Settings Page Actions (IMPLEMENTED)

| Button | Endpoint | What It Does |
|--------|----------|-------------|
| **Simulate Deploy** | POST /projects/:id/github/test-webhook | Triggers re-crawl immediately (no webhook needed) |
| **Detect Platform** | POST /projects/:id/github/redetect-deploy | Re-scans repo for vercel.json, netlify.toml, CNAME, GitHub Pages |
| **Verify Connection** | POST /projects/:id/github/verify | Pings GitHub API to confirm token is still valid |
| **Disconnect GitHub** | DELETE /projects/:id/github/disconnect | Removes webhook + deletes connection + resets sourceType to MANUAL |
...
Repo file tree:   GET  /repos/{owner}/{repo}/git/trees/{branch}?recursive=1
...
GitHub Pages:     GET  /repos/{owner}/{repo}/pages
```
...
API_BASE_URL=http://localhost:3000/api   # Used for webhook URL (use real domain in production)
...
# GitHub Integration (ALL IMPLEMENTED)
GET    /api/github/authorize                       Redirect to GitHub OAuth (JWT via query param)
GET    /api/github/callback                        Handle GitHub OAuth callback → redirect to settings
POST   /api/projects/:id/github/connect            Select repo + set up webhook + detect deploy platform
...
DELETE /api/projects/:id/github/disconnect           Disconnect GitHub + remove webhook from repo
POST   /api/projects/:id/fix-issue/:issueId         Auto-fix issue (PR for GitHub, API for WordPress)
POST   /api/github/webhook                          GitHub webhook receiver (push/deploy events)
POST   /api/projects/:id/github/test-webhook        Simulate deploy → trigger re-crawl (dev/testing)
POST   /api/projects/:id/github/redetect-deploy     Re-run deploy platform detection
POST   /api/projects/:id/github/verify              Verify GitHub token is still valid
```
